...
const app = express();
const isStack = (process.env.SERVICE || '').toLowerCase() === 'stack';
// Stack: web owns Railway PORT; API listens only on API_PORT (internal).
const port = isStack
  ? Number(process.env.API_PORT) || 3848
  : Number(process.env.API_PORT || process.env.PORT) || 3848;
